...
const SendEstimateModal = ({ 
  ...
  const handleSendViaText = async () => {
    try {
      ...
      const file = new File(
        [pdfBlob], 
        `Estimate-${estimate.estimateNumber || 'draft'}.pdf`, 
        { type: 'application/pdf' }
      );
      
      // Pre-filled message
      const shareMessage = `Hi ${estimate.clientName || 'there'}, please find attached your estimate${estimate.estimateNumber ? ' #' + estimate.estimateNumber : ''} for ${estimate.name || 'your project'}. Total: $${parseFloat(estimate.total || estimate.amount || 0).toFixed(2)}. This estimate is valid for 30 days. Thank you!`;
